...
class AutoPiano():
    def __init__(self, musical_path) -> None:
        self.musics = {}
        self.musical_path = musical_path
        self.state = "play"
        self.cur_song = 0
        self.stage = 0
        self.find_window = False
        self.musical = []

    def load_musics(self):
        musical_data = sorted(Path(self.musical_path).glob('*'))
        with open(musical_data[0], 'r') as f:
            return [[item for item in line.split(' ')] for line in f.readlines()]
    ...
    def auto_play(self):
        ...
        self.musical = self.load_musics()
        print(self.musical)
        for line in self.musical:
            if len(line) < 3:
                print('下一节')
                print(self.stage + 1)
                self.stage += 1
                continue
            while self.state == "pause" or str(win32gui.GetWindowText(
                    win32gui.GetForegroundWindow())).find("piano") == -1:
                time.sleep(1)
            if str(win32gui.GetWindowText(
                    win32gui.GetForegroundWindow())).find('piano') != -1 and self.find_window == False:
                pyautogui.click(770, 630, clicks=2, interval=0.2, duration=0.6)
                self.find_window = True
            hwsc = win32api.MapVirtualKey(VK_CODE[line[0]], 0)
            win32api.keybd_event(VK_CODE[line[0]], hwsc, 0, 0)
            time.sleep(float(line[1]) / 1000)
...
if __name__ == '__main__':
    musical_path = r'./musical'
    auto_piano = AutoPiano(musical_path)
    auto_piano.start()
    exit(0)
